...
| `update_quirk` | A recalled quirk is outdated or wrong — fix its content, type, tags, confidence, or source ref | `update_quirk("quirk:abc…", { content: "corrected fact", type: "decision" })` |
| `delete_quirk` | A quirk is fixed, obsolete, or no longer applies — remove it so it stops being recalled/injected | `delete_quirk("quirk:abc…")` |
...
4. **Delete a quirk** — `delete_quirk(id)` removes the quirk chunk from the vector store + keyword index and its audit entry from `quirks.jsonl`, so it is no longer recalled or auto-injected. Deleting (rather than updating) is the right call when a quirk is simply wrong or superseded — it keeps memory free of contradictions.

5. **Decay & lint** — Optional confidence decay ages quirks over time (`memory.decay.halfLifeDays`). `opencode-rag quirk lint` flags low-confidence, stale, and near-duplicate quirks; `opencode-rag quirk test "<text>"` reports whether a similar quirk has already been appended.

6. **Auto-capture (three modes)** — Reduce reliance on agent discipline by enabling automatic quirk extraction:
   ...
   - **Prompt enforcement** (`memory.promptEnforcement`): Replaces the soft "add_quirk when you discover a non-obvious fact" nudge with mandatory rules listing concrete triggers (build/test/type errors fixed, undocumented constraints, workarounds, etc.) — including mandatory `update_quirk` / `delete_quirk` hygiene for outdated stored quirks.
...
The plugin therefore default-exports the V2 shape — `id` + `setup` (OpenCode 1.x
support was dropped in v2.0.0):
...
async function setup(ctx: Plugin.Context) {
  return registerRagPluginV2(ctx); // registers V1-factory hooks via the context adapter
}
...
const plugin = {
  ...
  setup, // registers sidebar/keybindings when the TUI role loads it; no-op for the server role
};
...
- The `default` export MUST be an object with `id` **and** `setup` (or `effect`) — V2 rejects
  bare functions and V1-era `{ id, server }` / `{ id, tui }` shapes
...
> **Migration note:** OpenCode 1.x (`{ id, server }` / `{ id, tui }` entrypoints) is no longer
> supported as of v2.0.0. On V2, an old shape produces the schema error above.
...
2. The generated plugin file re-exports the default export from `node_modules/`:
...
`function`. For the TUI module substitute
`.opencode/plugins/rag-tui.js`; its default must expose `id` and `setup`.
