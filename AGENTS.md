...
- **LanceDB "partition N is empty, skipping" warnings**: benign once per index build (IVF KMeans on duplicate/degenerate vectors). Constant spam = retrain churn from a corrupted store (index commits never register — one new `_indices/<uuid>` dir per attempt). `repairIndexMetricOnce` guards: skips when `countIndexVersionDirs()` > 40, gives up after 3 failed attempts per process; rebuilds pass `optimize({ skipIndex: true })` to temp-store mid-run optimizes so the index is built once at the end. Fix for an affected workspace: delete rag_db + reindex. Note: u64-near `_versions/*.manifest` names are NORMAL (counter starts at u64::MAX-1 and decrements) — not corruption.
