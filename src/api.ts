...
export async function search(
  ...
  try {
    ...
    const rawResults = await retrieve(query, ctx.embedder, ctx.store, {
      ...
      symbolKeywordWeight: ctx.config.retrieval.hybridSearch?.symbolKeywordWeight,
      docKeywordDemotion: ctx.config.retrieval.hybridSearch?.docKeywordDemotion,
      testKeywordDemotion: ctx.config.retrieval.hybridSearch?.testKeywordDemotion,
